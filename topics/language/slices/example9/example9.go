...
func main() {
	...
	// Perform a linear traversal across the bytes, never making copies of the actual data but
	// still passing those bytes to the binary function for processing
	a := x[0]
	b := binary.LittleEndian.Uint16(x[1:3])
	c := binary.LittleEndian.Uint16(x[3:5])
	d := binary.LittleEndian.Uint16(x[5:9])

	// The result is zero allocation data access that is mechanically sympathetic with the hardware.
	fmt.Println(a, b, c, d)
}
